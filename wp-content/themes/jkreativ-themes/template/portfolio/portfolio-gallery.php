<?php
	...
	$itemwidthbase 	= vp_metabox('jkreativ_portfolio_sidecontent.grid.0.item_width', null, JEG_PAGE_ID);
	$itemheightdim 	= null;
	$itemheightbase = '';
	
	$overwritewidth = vp_metabox('jkreativ_portfolio_sidecontent.image_fullwidth', null, JEG_PAGE_ID);
	
	if($gallerylayout === "normal") {
		$itemheightdim = floatval ( vp_metabox('jkreativ_portfolio_sidecontent.grid.0.item_height', null, JEG_PAGE_ID) );
		$itemheightbase = $itemheightdim * $itemwidthbase;
	} else if($gallerylayout === 'justified') {
		$itemheightbase = floatval ( vp_metabox('jkreativ_portfolio_sidecontent.grid.0.justified_item_height', null, JEG_PAGE_ID) ); 
	}
	
	$usemargin =  vp_metabox('jkreativ_portfolio_sidecontent.grid.0.use_margin', null, JEG_PAGE_ID);
	$marginsize = 0;
	$althidetitle = vp_metabox('jkreativ_portfolio_sidecontent.grid.0.photoswipe_setting.0.photoswipe_hide_title', null, JEG_PAGE_ID);
	$notloadedclass = 'notloaded';
    $showimagetitle = vp_metabox('jkreativ_portfolio_sidecontent.show_image_title', 0, JEG_PAGE_ID);
	
	if($usemargin) {
		$marginsize = vp_metabox('jkreativ_portfolio_sidecontent.grid.0.margin_size',  null, JEG_PAGE_ID);	
	...
	if($mediagallery) {
		...
		for($key = $bottomlimit ; $key < $uplimit ; $key++)
		{
			...
			if($value['type'] === 'image') {
				...
				if($overwritewidth) {
                    echo
                        "<div class='imageholder $showimagetitleclass'><div class='fullimgwrapper'><img src='$image'/>" . $showimagetitletag . "</div></div>";
				} else {
				...
				if($value['type'] === 'soundcloud') {
					$value['mediaurl'] = "https://w.soundcloud.com/player/?url=" . jeg_encodeURIComponent($value['mediaurl']);
				}
				...
				if(isset($value['videomp4']) && $value['videomp4'] !== '') {
					$videomp4 = "<source type='video/mp4' src='{$value['videomp4']}' />";
				}
				
				if(isset($value['videowebm']) && $value['videowebm'] !== '') {
					$videowebm = "<source type='video/webm' src='{$value['videowebm']}' />";
				}
		    	    	    	
				if(isset($value['ogg']) && $value['ogg'] !== '') {
				...
				if($overwritewidth) {
					...
					echo 
					"<div class='imggalitem {$notloadedclass}'  data-width='{$value['width']}' data-height='{$value['height']}' style='padding: {$marginsize}px;'>
						<a href='#html5popup' data-type='{$value['type']}' style='margin: 0px;' title='{$videoname}'>
							<img src='{$thumbnail}' alt=''>
							<div class='videooverlay'></div>
						</a>
